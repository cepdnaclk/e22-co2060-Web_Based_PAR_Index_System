...
public class CaseController {
    ...
    // Automates landmark-point placement right after upload — see
    // triggerMLPredictionAsync() below, where this now runs alongside the
    // existing total-PAR prediction instead of requiring a separate manual
    // trigger.
    ...
    private void triggerMLPredictionAsync(Long caseId, User user) {
        mlExecutor.submit(() -> {
            try {
                ...
                // Automate landmark-point placement: previously a clinician
                // had to place every point by hand in the 3D viewer (or
                // trigger prediction manually) even though the ML pipeline
                // already existed. Now runs automatically as soon as all 3
                // models are uploaded, same trigger as the total-PAR
                // prediction below. Points still arrive as ML_PREDICTED /
                // unconfirmed — GeometricPARService still requires a
                // clinician to confirm them before auto-calculate will use
                // them, so this automates placement, not clinical sign-off.
                try {
                    mlPredictionService.predictForCase(caseId, user);
                    log.info("Auto landmark prediction completed for case {}", caseId);
                } catch (Exception e) {
                    // Non-fatal: total-PAR prediction below is independent
                    // and should still run even if landmark prediction fails
                    // (e.g. ml-service temporarily down, a corrupt mesh).
                    log.warn("Auto landmark prediction failed for case {}: {}", caseId, e.getMessage());
                    auditService.log(user, "ML_LANDMARK_PREDICTION_FAILED", "OrthoCase", caseId,
                            "reason=" + e.getMessage());
                }
